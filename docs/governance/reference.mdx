...
id: reference
slug: /governance/reference
---

import useBaseUrl from "@docusaurus/useBaseUrl";

Cryptex protocol is goverened and upgraded by CTX token holders, using three distinct components; the CTX token, governance module, and Timelock. Together, these contracts allow the community to propose, vote, and implement changes to the Cryptex protocol.

Any addresses with more than 100k CTX delegated to it may propose governance actions, which contain finished, executable code. When a proposal is created, the community can cast their votes during a 3 day voting period. If a majority, and at least 400k votes are cast for the proposal, it is queued in the Timelock, and may be executed in a minimum of 2 days.
...
<img alt="" src={useBaseUrl("img/gov_diagram-1.png")} />
...
Delegate votes from the sender to the delegatee. Users can delegate to 1 address at a time, and the number of votes added to the delegatee’s vote count is equivalent to the balance of CTX in the user’s account. Votes are delegated from the current block and onward, until the sender delegates again, or transfers their CTX.
...
The sender must hold more CTX than the current proposal threshold (proposalThreshold()) as of the immediately previous block. The proposal can have up to 10 actions (based on proposalMaxOperations()).
