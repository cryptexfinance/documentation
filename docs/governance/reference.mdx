...
Cryptex protocol is governed and upgraded by CTX token holders, using three distinct components; the CTX token, governance module, and Timelock. Together, these contracts allow the community to propose, vote, and implement changes to the Cryptex protocol.
